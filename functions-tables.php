...
//			$chart_data[] = sprintf("{ 'd' : '%s', 'p' : %s }", mb_convert_case( $date_m[ date('n', strtotime($row['mydate']) ) - 1 ] , MB_CASE_TITLE, "UTF-8"), $price);
            ...
            <select id="form-select2" name="district" class="form-control" aria-label="Федеральный округ">
	          ...
	          $params_output = '';
		        while($row = $result_filter2->fetch_row()) {
		          ...
		          if( $filter_district == $row[0] ) { $param_in_array = true; }
	            if(!empty($row[0])) {
          ...
          <div class="col-md-4">
            ...
            <select id="form-select2" name="country" class="form-control" aria-label="Страна (базис)">
              ...
              while($row = $result_filter2->fetch_row()) {
	              ...
	              if( $filter_country == $row[0] && $filter_basis == $row[1] ) { $param_in_array = true; }
  ...
  <script>
      ...
      const chartData = [
		  <?php echo join($chart_data, ',' . PHP_EOL) ?>
      ];
      const color = '#7fbc2b';
...
function get_all_records_front($country) {
	...
	if (isset($country) && $country == 'all') {
		$table = 'oleo_price_world';
		$table_headings = array('Наименование', 'Страна', 'Базис', 'Торги на', 'Дата изм.', 'Цена', 'Ед. изм.', 'За нед.', 'За мес.', 'Мин.', 'Макс.', 'График', 'Файл');
		$table_headings_hidden = array(0,1,0,0,1,1,2,1,1,1,1,0,0);
	} else {
		$table = 'oleo_price_russia';
		$table_headings = array('Наименование', 'Страна', 'Округ', 'Дата изм.', 'Цена (руб/т)', 'За нед.', 'За мес.', 'Мин. за 24 мес.', 'Макс. за 24 мес.', 'График', 'Файл');
...
//	  $query = "SELECT *, t1.id as id FROM ".$table." AS t1, ".$table." AS t2 WHERE date IN (SELECT max(date) FROM ".$table." group by name, basis, delivery_date ORDER BY delivery_date) ORDER BY name DESC LIMIT 500";
	  $query = "SELECT `id`, `date`, `name`, `country`, `basis` FROM oleo_price_world
...
function export_csv($id, $country, $chart) {
	...
	if (isset($id) && $id != 0) {
	   ...
	   */
		...
		// запрос полей, по которым выводим данные по индексу, полученному по ссылке
		$pre_query = "SELECT *, DATE_FORMAT(date, '%d.%m.%Y') as date FROM " . $table . " WHERE id = " . $stat_id . ";";
		//  echo $pre_query;
		$pre_result = $mysqli->query( $pre_query ) or die('Нет результатов по запросу');
		$pre_row = $pre_result->fetch_assoc();
...
?>
            ...
            <? endif; ?>
              <p><?= $pre_row['date']; ?> г. цена на <?= mb_lcfirst($chart_stats[0]['name']) ?> (<?=  $chart_stats[0]['basis'] ?>) с поставкой на <? //strtolower( date('F', $pre_row['mydate']) ) ?> <?= $date_m[ date('n', strtotime($chart_stats[0]['mydate']) ) - 1 ] . ' ' . date('Y', strtotime($chart_stats[0]['mydate']) ) ?> составила <span title="<?= $chart_stats[0]['units'] ?>"><?= $chart_stats[0]['price'] ?></span> $/т,
              что на 28 $/т ниже предыдущего значения от 16.03.2021 г. (1449 $/т).</p>

              <p>Максимальная цена поставки на май за последний месяц составляла 1450 $/т
              (09.03.2021 г.). Минимальная цена - 1262 $/т (18.02.2021 г.).</p>

              <p>От <?= $pre_row['date']; ?> стоимость поставок на:<br>
                - май составила 1421 $/т<br>
                - июль составила 1341 $/т<br>
                - август составила 1316 $/т<br>
                - сентябрь составила 1299 $/т<br>
                - ноябрь составила 1274 $/т<br>
              </p>
...
LIMIT 1000";
  ...
  if ($Russia) {
    ...
    //   $chart_data2 = array();
    $table_rows = '';
    $table_allowed_rows = array('Начальные запасы, тыс. т', 'Производство, тыс. т', 'Импорт, тыс. т', 'Всего ресурсы к распределению, тыс. т', 'Внутреннее потребление, тыс. т', 'Экспорт, тыс. т', 'Конечные запасы, тыс. т');
    $table_marked_rows = array('Пищевое внутреннее потребление, тыс. т', 'Внутреннее потребление на кормовые отходы, тыс. т', 'Промышленное внутреннее потребление, тыс. т');
  ...
  <div class="chart">
    <header>
	  ...
	  <? endif; ?>
          ...
          <?
            for($i = 0; $i < count($chart_fts_data); $i++) :
          ?>
            <script>
                const chartDataFTS<?= $i ?> = [
                <?
                    echo join($chart_fts_data[ $i ], ',' . PHP_EOL);
                ?>
                ];
  ...
  <script>
      ...
      const chartData<?= $i ?> = [
		  <?php echo join($chart_data[$i], ',' . PHP_EOL) ?>
      ];
